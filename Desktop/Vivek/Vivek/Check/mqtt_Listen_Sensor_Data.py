...
def DHT22_Humidity_Data_Handler(payload):
	...
	cursor = conn.cursor()
	cursor.execute("insert into iot.listen_sensor_data (sensortype, sensordate, sensorvalue) values (%s,%s,%s);",(SensorID, Data_and_Time,Humidity ))
	conn.commit()
...
def DHT22_Temp_Data_Handler(payload):
	...
	conn = psycopg2.connect(conn_string)
	# conn.cursor will return a cursor object, you can use this cursor to perform queriesTemperature
	cursor = conn.cursor()
	cursor.execute("insert into iot.listen_sensor_data (sensortype, sensordate, sensorvalue) values (%s,%s,%s);",(SensorID, Data_and_Time,Temperature ))
	conn.commit()
